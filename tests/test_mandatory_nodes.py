...
TASK_BUILD_SALES_SUGGESTION = "build_sales_suggestion"
...
class TestMandatoryNodesEnforcement:
    ...
    def test_build_final_plan_adds_downstream_mandatory_nodes_after_fetch(self):
        """If behavior is fetched in-plan, classify_intent and anti_disturb_check must follow."""
        context = AgentContext(
            user_id="user_001",
            sku="8WZ01CM1",
        )
        ...
        assert fetch_behavior_idx < classify_intent_idx < anti_disturb_idx < retrieve_rag_idx

    def test_build_final_plan_appends_sales_suggestion_for_sales_graph(self):
        """sales/graph context should append build_sales_suggestion as the last node."""
        context = AgentContext(
            user_id="user_001",
            sku="8WZ01CM1",
        ...
        assert TASK_BUILD_SALES_SUGGESTION in final_plan
        assert final_plan[-1] == TASK_BUILD_SALES_SUGGESTION
        assert final_plan.index(TASK_GENERATE_COPY) < final_plan.index(
...
class TestBusinessLogicValidation:
    ...
    @pytest.mark.asyncio
    async def test_execute_plan_keeps_sales_suggestion_after_block(self, monkeypatch):
        """sales/graph should still build suggestion pack when anti-disturb blocks send."""
        from app.agents.graph import sales_graph as sales_graph_module

        async def fake_anti_disturb(context):
            context.extra["allowed"] = False
            context.extra["anti_disturb_blocked"] = True
            return context

        async def fake_build_sales_suggestion(context):
            context.extra["sales_suggestion"] = {
                "recommended_action": "soft_check_in",
                "send_recommendation": {"suggested": False},
            }
            return context
        ...
        result = await sales_graph_module._execute_plan(
            context,
            [TASK_ANTI_DISTURB_CHECK, TASK_BUILD_SALES_SUGGESTION],
        )

        assert result.extra["anti_disturb_blocked"] is True
        assert result.extra["sales_suggestion"]["recommended_action"] == "soft_check_in"
