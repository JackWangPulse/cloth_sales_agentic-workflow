...
class VectorStore:
    ...
    async def search_async(self, query: str, top_k: int = 5) -> List[Tuple[str, float]]:
        """Async search for use inside running event loops."""
        if not query or not query.strip():
            return []

        if self.use_incremental:
            return await self._search_incremental_async(query, top_k)
        ...
        embedding_client = get_embedding_client()
        query_embeddings = await embedding_client.embed_texts([query])
        ...
        if abs(query_norm_after - 1.0) > 0.01:
            logger.warning(
                f"[VECTOR_STORE] Query vector normalization may have failed! "
                f"Expected norm=1.0, got {query_norm_after:.4f}"
            )
        else:
            logger.info(
    ...
    async def _search_incremental_async(self, query: str, top_k: int) -> List[Tuple[str, float]]:
        """Async search using base+delta incremental strategy."""
        logger.info(f"[VECTOR_STORE] Incremental search: '{query[:50]}...' (top_k={top_k})")
        ...
        if self.base_index and self.base_index.ntotal > 0:
            base_top_k = min(top_k * 2, self.base_index.ntotal)
            distances, indices = self.base_index.search(query_vector, base_top_k)
