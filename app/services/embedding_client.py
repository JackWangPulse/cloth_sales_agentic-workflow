...
class EmbeddingClient:
    ...
    async def aclose(self) -> None:
        """Close the pooled async client."""
        try:
            await self._async_client.aclose()
        except Exception:
            logger.debug("Failed to close embedding async client cleanly", exc_info=True)
    ...
    async def embed_texts(self, texts: List[str]) -> List[List[float]]:
        ...
        async def process_batch(batch_index: int, batch: List[str]) -> List[List[float]]:
        ...
        batch_results = await asyncio.gather(
            *(process_batch(index, batch) for index, batch in enumerate(batches))
        )
        all_embeddings = [embedding for batch in batch_results for embedding in batch]
...
async def close_embedding_client() -> None:
    """Close the global embedding client if it exists."""
    global _embedding_client
