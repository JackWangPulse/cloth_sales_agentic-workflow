...
from app.services.langsmith_service import traceable_async_generator, traceable_sync

logger = logging.getLogger(__name__)
_llm_duration_ms_var: contextvars.ContextVar[float] = contextvars.ContextVar(
    "llm_duration_ms",
    default=0.0,
)
...
def reset_llm_timing() -> None:
    """Reset per-request accumulated LLM duration."""
    _llm_duration_ms_var.set(0.0)


def add_llm_duration_ms(duration_ms: float) -> None:
    """Accumulate per-request LLM duration."""
    _llm_duration_ms_var.set(_llm_duration_ms_var.get() + max(duration_ms, 0.0))


def get_llm_duration_ms() -> float:
    """Read per-request accumulated LLM duration."""
    return _llm_duration_ms_var.get()
...
class LLMClient:
    ...
    def __init__(self) -> None:
        ...
        self._sync_client = httpx.Client(timeout=self._timeout, limits=limits)
        self._async_client = httpx.AsyncClient(timeout=self._timeout, limits=limits)

    def close(self) -> None:
        """Close the pooled sync client."""
        try:
            self._sync_client.close()
        except Exception:
            logger.debug("Failed to close sync LLM client cleanly", exc_info=True)

    async def aclose(self) -> None:
        """Close both pooled clients."""
        self.close()
        try:
            await self._async_client.aclose()
        except Exception:
            logger.debug("Failed to close async LLM client cleanly", exc_info=True)

    @traceable_sync("llm.generate", run_type="llm", tags=["llm"])
    def generate(self, prompt: str, **kwargs: Any) -> str:
        ...
        if not self.settings.llm_api_key or not self.settings.llm_base_url:
            ...
            text = f"[{self.settings.llm_model}:{suffix}] {prompt.strip()}"
            add_llm_duration_ms((perf_counter() - start) * 1000)
            return text
        ...
        add_llm_duration_ms(duration_ms)
    ...
    @traceable_async_generator("llm.stream_chat", run_type="llm", tags=["llm"])
    async def stream_chat(
        ...
        **kwargs: Any
        ...
        if not self.settings.llm_api_key or not self.settings.llm_base_url:
            ...
            add_llm_duration_ms((perf_counter() - start_time) * 1000)
            return
...
async def close_llm_client() -> None:
    """Close the global LLM client if it exists."""
    global _llm_client
    client = globals().get("_llm_client")
    if client is None:
        return
    await client.aclose()
    _llm_client = None
