...
from app.services.langsmith_service import traceable_async
...
ACTION_ASK_SIZE = "ask_size"  # 尺码/版型咨询
ACTION_REASSURE_COMFORT = "reassure_comfort"  # 材质/上身感受保证
ACTION_SCENE_RELATE = "scene_relate"  # 场景/搭配关联
...
def choose_recommended_action(
    ...
    if intent_level == INTENT_HIGH:
        if has_click_size_chart:
            return (
                ACTION_ASK_SIZE,
                "用户已查看尺码信息，建议主动询问尺码或版型偏好以推进购买",
            )
        elif has_favorite:
            return (
                ACTION_MENTION_STOCK,
                "用户已收藏商品，建议提醒库存或颜色选择以促进下单",
            )
        else:
            return (
                ACTION_ASK_SIZE,
                "用户购买意图强烈，建议询问尺码或版型偏好以推进购买",
            )
        ...
        if scene:
            return (
                ACTION_SCENE_RELATE,
                f"用户有一定兴趣，建议推荐{scene}场景穿搭方式",
            )
        else:
            return (
                ACTION_REASSURE_COMFORT,
                "用户有一定兴趣，建议强调材质手感和上身感受以增强信心",
            )
...
def build_send_recommendation(
    ...
    if confidence == "high":
        ...
        next_step_map = {
            ACTION_ASK_SIZE: "根据用户的尺码或版型偏好，推荐更合适的款式",
            ACTION_ASK_CONCERN_TYPE: "根据用户顾虑类型，提供针对性解答",
            ACTION_REASSURE_COMFORT: "进一步强调材质手感和上身感受，提供试穿或搭配建议",
            ACTION_SCENE_RELATE: "根据用户穿着场景，推荐搭配方案",
...
def _build_message_pack_prompt_v2(
    ...
    strategy_descriptions = {
        ...
        MESSAGE_STRATEGY_ASK_SIZE: "询问尺码/版型（如'您平时喜欢修身一点还是宽松一点？'）",
        MESSAGE_STRATEGY_REASSURE_COMFORT: "材质/上身感受保证（如'这款面料上身会比较舒服'）",
        MESSAGE_STRATEGY_SCENE_RELATE: "场景/搭配推荐（如'适合通勤或日常搭配'）",
    ...
    prompt_parts.append("3. 每条消息必须包含行动建议关键词（尺码/版型/搭配/场景/库存/优惠/试穿等）")
    ...
    prompt_parts.append("这件黑色针织衫上身会比较舒服，您平时更喜欢修身一点还是宽松一点？")
    prompt_parts.append("这条牛仔裤适合日常通勤搭配，您觉得这个风格怎么样？")
    prompt_parts.append("")
...
def _build_message_pack_prompt(
    ...
    # 动作提示
    action_hints = {
        ACTION_ASK_SIZE: "询问尺码/版型（如'您平时喜欢修身一点还是宽松一点？'）",
        ACTION_REASSURE_COMFORT: "强调材质和上身感受（如'这款面料穿着会比较舒服'）",
        ACTION_MENTION_STOCK: "提醒库存（如'库存不多'）",
        ACTION_MENTION_PROMO: "提及优惠（如果有促销活动）",
        ACTION_SCENE_RECOMMENDATION: "场景/搭配推荐（如'适合通勤或日常搭配'）",
...
def _parse_llm_message_pack(response: str, recommended_action: str) -> List[str]:
    ...
    for line in lines:
        ...
        # 检查是否包含行动建议关键词
        action_keywords = {
            ACTION_ASK_SIZE: ["码", "尺码", "版型"],
            ACTION_REASSURE_COMFORT: ["舒适", "舒服", "上身", "面料"],
            ACTION_MENTION_STOCK: ["库存", "现货"],
            ACTION_MENTION_PROMO: ["优惠", "活动", "促销"],
            ACTION_SCENE_RECOMMENDATION: ["适合", "场景", "搭配"],
...
@traceable_async("sales_graph.build_suggestion_pack", tags=["sales_graph", "suggestion"])
    ...
    if recommended_action == ACTION_ASK_SIZE:
        playbook.extend([
            FollowupPlaybookItem(
                condition="顾客说尺码或版型不确定",
                reply="你平时这类衣服更喜欢修身一点还是宽松一点？我帮你更准一点～",
            ),
            FollowupPlaybookItem(
                condition="顾客说再看看",
                reply="好的不急～你如果在意版型、手感或搭配，我也可以给你更具体的建议～",
            ),
        ])
        ...
        playbook.extend([
            ...
            FollowupPlaybookItem(
                condition="顾客说担心上身效果",
                reply="这款版型和面料都比较友好，很多人反馈上身舒服，你平时更在意版型还是手感呀？",
            ),
        ])
    elif recommended_action == ACTION_REASSURE_COMFORT:
        playbook.extend([
        ...
        playbook.extend([
            ...
            FollowupPlaybookItem(
                condition="顾客问适合什么场景",
                reply="这款挺适合日常通勤和轻松出门穿的，你平时会更偏通勤还是休闲一点呀？",
            ),
        ])
    elif recommended_action == ACTION_MENTION_STOCK:
        playbook.extend([
            FollowupPlaybookItem(
                condition="顾客问库存",
                reply="我帮你看看现在还有哪些尺码和颜色，你平时通常拿什么码呀？",
            ),
            FollowupPlaybookItem(
                condition="顾客说再看看",
                reply="好的不急～库存这边还有，你如果确定要我可以帮你先留一下～",
            ),
        ])
