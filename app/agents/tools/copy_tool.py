...
from app.services.graph_task_cache_service import (
    COPY_LOCK_TTL_SECONDS,
    acquire_lock,
    build_copy_cache_key,
    build_copy_lock_key,
    get_cached_copy_payload,
    release_lock,
    set_cached_copy_payload,
    wait_for_json_cache,
)
...
@traceable_async("sales_graph.generate_copy", tags=["sales_graph", "copy"])
async def generate_marketing_copy(
    ...
    **kwargs: Any,
    ...
    cache_key = build_copy_cache_key(context, max_length)
    lock_key = build_copy_lock_key(context, max_length)
    lock_token = None

    cached_payload = get_cached_copy_payload(context, max_length)
    if cached_payload:
        logger.info("[COPY_TOOL] Cache hit for sku=%s intent=%s", context.sku, context.intent_level)
        context.extra["copy_llm_used"] = bool(cached_payload.get("llm_used", False))
        context.extra["copy_strategy"] = str(cached_payload.get("copy_strategy", ""))
        context.add_message("assistant", str(cached_payload.get("message", "")))
        return context

    lock_token = acquire_lock(lock_key, COPY_LOCK_TTL_SECONDS)
    if lock_token is None:
        logger.info("[COPY_TOOL] Lock busy for sku=%s intent=%s, waiting for cache", context.sku, context.intent_level)
        waited_payload = wait_for_json_cache(cache_key)
        if isinstance(waited_payload, dict):
            context.extra["copy_llm_used"] = bool(waited_payload.get("llm_used", False))
            context.extra["copy_strategy"] = str(waited_payload.get("copy_strategy", ""))
            context.add_message("assistant", str(waited_payload.get("message", "")))
            return context
        logger.info("[COPY_TOOL] Cache wait timed out for sku=%s, continue without lock", context.sku)
    ...
    try:
        ...
        set_cached_copy_payload(
            ...
            message=copy_text,
            llm_used=llm_used,
            copy_strategy=context.extra["copy_strategy"],
        )
        ...
        try:
            ...
            context.extra["copy_llm_used"] = False
            context.extra["copy_strategy"] = _get_strategy_description(context.intent_level)
            context.add_message("assistant", copy_text)
            set_cached_copy_payload(
                context=context,
...
def _get_strategy_description(intent_level: str) -> str:
    ...
    strategies = {
        "high": "主动推进（询问尺码或版型/提醒库存或颜色）",
        "hesitating": "消除顾虑（轻量提问）",
        "medium": "场景/搭配推荐",
        "low": "轻量提醒（不施压）",
    }
    return strategies.get(intent_level, "场景/搭配推荐")
