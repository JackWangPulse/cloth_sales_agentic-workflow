...
async def plan_sales_flow(context: AgentContext) -> List[str]:
    ...
    if context.extra.get("include_sales_suggestion", False):
        plan.append(TASK_BUILD_SALES_SUGGESTION)
        logger.debug("[PLANNER] Added: build_sales_suggestion (sales/graph mode)")
...
def build_final_plan(
    ...
    # Consider state that will exist after earlier nodes in this same plan run.
    will_have_behavior_summary = (
        context.behavior_summary is not None
        or TASK_FETCH_BEHAVIOR_SUMMARY in custom_plan
    )
    will_have_intent_level = (
        context.intent_level is not None
        or TASK_CLASSIFY_INTENT in custom_plan
    )
    ...
    if will_have_intent_level or will_have_behavior_summary:
    ...
    if context.extra.get("include_sales_suggestion", False):
        optional_from_custom = [
            node for node in optional_from_custom if node != TASK_BUILD_SALES_SUGGESTION
        ]
        optional_from_custom.append(TASK_BUILD_SALES_SUGGESTION)
