...
for i in range(15):
    try:
        ...
        if r.status_code == 200:
            print(f"鉁?Server is running! (attempt {i+1})")
            break
    except:
        time.sleep(1)
        if i < 14:
    ...
    print("鉁?Server did not start in time")
...
for path, name in endpoints:
    try:
        ...
        status = "鉁? if r.status_code == 200 else "鉁?
        ...
        print(f"鉁?{name:20} {path:20} Error: {e}")
